...
!!!!! variables Elmer
  ...
  REAL(kind=dp) :: Lambda,hscale,vscale
...
!! autres variables
  ...
  real(kind=dp),allocatable,dimension(:) :: NodalViscosityb,NodalRegb
...
!!!!!!!!!!! get Solver Variables
       ...
       SquareFormulation=GetLogical( SolverParams, 'SquareFormulation', Found)
        ...
        Lambda = GetConstReal( SolverParams,'Lambda',Found)
           IF(.NOT.Found) THEN
                ...
                Lambda = 0.0
           END IF
        hscale = GetConstReal( SolverParams,'Horizontal Scale',Found)
           IF(.NOT.Found) THEN
                CALL WARN(SolverName,'Keyword >hscale< not found in section >Solver<')
                CALL WARN(SolverName,'Taking default value hscale=1.0')
                hscale = 1.0
           END IF
        vscale = GetConstReal( SolverParams,'Vertical Scale',Found)
           IF(.NOT.Found) THEN
                CALL WARN(SolverName,'Keyword >vscale< not found in section >Solver<')
                CALL WARN(SolverName,'Taking default value vscale=1.0')
                vscale = 1.0
           END IF
 ...
 ! Get variables needed by the Solver
    ...
    DO e=1,Solver % NumberOfActiveElements
             ...
             Do p=1,n
              ...
              If (Lambda /= 0.0) THEN
                        NodalRegb(1:n)=NodalRegb(1:n)+&
                          s*Lambda*(SUM(dBasisdx(1:n,1)*Values(Perm(NodeIndexes(1:n))))*dBasisdx(1:n,1))*hscale
                        NodalRegb(1:n)=NodalRegb(1:n)+&
                          s*Lambda*(SUM(dBasisdx(1:n,2)*Values(Perm(NodeIndexes(1:n))))*dBasisdx(1:n,2))*hscale
                    IF (DIM.eq.3) THEN
                        NodalRegb(1:n)=NodalRegb(1:n)+&
                          s*Lambda*(SUM(dBasisdx(1:n,3)*Values(Perm(NodeIndexes(1:n))))*dBasisdx(1:n,2))*vscale
                    END IF
              END IF
          ...
          db(NodeIndexes(1:n)) = db(NodeIndexes(1:n)) + nodalViscosityb(1:n) + NodalRegb(1:n)
       End Do ! on elements
