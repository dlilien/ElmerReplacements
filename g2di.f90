...
SUBROUTINE Grid2DInterpolator( Model,Solver,dt,TransientSimulation )
   ...
   INTEGER,parameter :: io=20, nSkipDefault = 0
   INTEGER :: ok, Nx, Ny, Nb, Nbaux, OutNode, dumy
   INTEGER :: i, j, k, l, kmin, NoVar, nSKIP
   ...
   DO WHILE(GotVar)
      ...
      WRITE (ParaName,'(A,I0,A)') 'Variable ',NoVar,' nSkip'
      nSkip = ListGetInteger( Params, TRIM(ParaName), Found )
      IF (.NOT.Found) then
         nSkip = nSkipDefault
         WRITE(message,'(A,A,A)')'Keyword <',Trim(ParaName), & 
              '> not found, using 0'
         CALL INFO(SolverName, Message, Level=3)
      END IF
